...
# --- Initilisation de l'application FastAPI ---
app = FastAPI(
    title="MovieLens API",
...
async def root():
    return {"message": "API MoviLens opérationnelle. "}
...
@app.get(
    ...
    description="Retourne une liste de films avec la pagination des filtres optionnels par titre ou genre.",
    response_description="Liste de films",
    response_model=List[schemas.MovieSimple],
    tags=["films"],
)
def list_movies(
    skip: int = Query(0, ge=0, description="Nombre de résultats à l'ignorer"),
...
@app.get(
    ...
    description= "Retourne une liste d'évaluations avec pagination et filtres optionnels (film, utilsateur, note min).",
...
def list_ratings(
    ...
    limit: int = Query(100, le=1000, description="Nombre maximal de résultats à retourner"),
    movie_id: Optional[int] = Query(None, description="Filter par ID de film"),
    user_id: Optional[int] = Query(None, description="Filter par ID utilisateur"),
    min_rating: Optional[float] = Query(None, ge=0.0, le=5.0, description="Filter les notes supérieures égales à cette valeur"),
    db: Session = Depends(get_db)
):
...
@app.get(
    ...
    summary="Obtenir des tag spécifique",
...
def list_tags(
    ...
    result = helpers.get_tags(db, skip=skip, limit=limit, movie_id=movie_id, user_id=user_id)
...
def read_link(
    ...
    if result in None:
        raise HTTPException(
            status_code=404,
            detail=f"Aucun lien trouvé par le film avec l'ID {movie_id}"
        )
    return result
