...
# %%
# Tester la récupération de quelques évaluations (ratigs)

Ratings = db.query(Rating).limit(5).all()

for rating in Ratings : 
    print(f"User ID : {rating.userId}, Movie ID : {rating.movieId}, Rating ID : {rating.rating}, Timetamp : {rating.timestamp}")

# %%
# Film avec une note supérieur ou égale à 4 
...
# %%
# lorsque l'on ajoute la condition de jointure (pas nécessaire car l'on avait dea définit dans model)
hight_rated_movies = (
    db.query(Movie.title, Rating.rating)
    .join(Rating)
