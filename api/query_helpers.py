...
def get_movies(db: Session, skip: int = 0, limit: int = 100, title: str = None, genre: str = None) : 
    """Récupéré une liste de films avec filtres optionnels. """
...
def get_tags(
    ...
    if user_id is not None : 
        query = query.filter(modeks.Tag.userId == user_id)
...
def get_link_count(db: Session):
    """Retourne le nombre total de lien. """
